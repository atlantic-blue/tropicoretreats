...
- Auth flows: `ALLOW_USER_SRP_AUTH`, `ALLOW_REFRESH_TOKEN_AUTH`, `ALLOW_ADMIN_USER_PASSWORD_AUTH` (for CLI testing in Plan 02)
...
`cd /Users/juliantellez/github.com/atlantic-blue/tropicoretreats/infra && terraform validate` passes
  ...
  <done>
cognito.tf exists with User Pool (admin-only signup) and App Client (no secret, SRP + admin password auth flows).
  </done>
</task>
...
**Protected Route with Integration:**

1. Add `aws_apigatewayv2_integration.get_leads`:
   - integration_type = "AWS_PROXY"
   - integration_uri = aws_lambda_function.create_lead.invoke_arn (reuse existing Lambda)
   - integration_method = "POST"
   - payload_format_version = "2.0"

2. Add `aws_apigatewayv2_route.get_leads`:
   - route_key = "GET /leads"
   - authorization_type = "JWT"
   - authorizer_id = aws_apigatewayv2_authorizer.cognito.id
   - target = "integrations/${aws_apigatewayv2_integration.get_leads.id}"

**Expected behavior:** The create_lead Lambda only handles POST requests, so GET requests will return 405 Method Not Allowed. This is correct - it proves the authorizer is working (401 without token, 405 with valid token). The actual GET /leads handler will be implemented in Phase 5 (Admin Dashboard).
  </action>
  <verify>
...
2. No errors in plan output
  ...
  <done>
JWT authorizer configured referencing Cognito User Pool. GET /leads route uses JWT authorization_type with authorizer_id. Integration points to create_lead Lambda (will return 405 until Phase 5 adds GET handler).
  </done>
</task>
...
3. **Configuration correct:**
   ...
   - App Client has ALLOW_ADMIN_USER_PASSWORD_AUTH for CLI testing
   - JWT authorizer references correct issuer and audience
</verification>

<success_criteria>
- Cognito User Pool deployed with admin-only user creation
- App Client deployed with no secret and SRP + admin password auth flows
