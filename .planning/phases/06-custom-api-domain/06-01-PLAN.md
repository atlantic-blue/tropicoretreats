...
<task type="auto">
  ...
  <files>infra/api/main.tf, infra/api/outputs.tf</files>
  <action>
...
- `curl -I https://api.tropicoretreat.com/v1/leads` returns HTTP response (401 or 200)
  ...
  <done>
Custom domain is live, DNS resolves, API responds at api.tropicoretreat.com/v1.
  </done>
</task>
...
3. **Rate Limiting Active:**
   ...
   - (Optional) Load test to trigger 429 response

4. **Existing Functionality Unchanged:**
   - Contact form submission creates lead
   - Admin can log in and view leads
   - Old execute-api endpoint still works (backwards compatibility during transition)
